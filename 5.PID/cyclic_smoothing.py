...
def smooth(path, weight_data = 0.1, weight_smooth = 0.1, tolerance = 0.00001):
    ...
    while total_change>tolerance:
        total_change = 0
        for i in range(n):
            change_x = weight_data * (path[i][0] - newpath[i][0]) + weight_smooth * (newpath[(i-1)%n][0] + newpath[(i+1)%n][0] - 2.0 * newpath[i][0])
            change_y = weight_data * (path[i][1] - newpath[i][1]) + weight_smooth * (newpath[(i-1)%n][1] + newpath[(i+1)%n][1] - 2.0 * newpath[i][1])
            newpath[i][0] += change_x
            newpath[i][1] += change_y
            total_change += abs(change_x)+abs(change_y)
    return newpath
