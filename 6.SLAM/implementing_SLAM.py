...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    #
    #
    nd = 2 * (N + num_landmarks)
    omega = matrix()
    omega.zero(nd, nd)
    xi = matrix()
    xi.zero(nd, 1)
    m = matrix([[1, 0], [0, 1]])
    m = m.expand(nd, nd, [0, 1], [0, 1])
    omega = omega + m
    b = matrix([[50], [50]])
    b = b.expand(nd, 1, [0, 1], [0])
    xi = xi + b
    for i in range(len(data)):
        ...
        m = m.expand(nd, nd, [2 * i, 2 * i + 2], [2 * i, 2 * i + 2])
        omega = omega + m
        b = matrix([[-1 * dx / motion_noise], [dx / motion_noise]])
        b = b.expand(nd, 1, [2 * i, 2 * i + 2], [0])
        xi = xi + b
        dy = data[i][1][1]
        m = matrix(
            [
        ...
        m = m.expand(nd, nd, [2 * i + 1, 2 * i + 3], [2 * i + 1, 2 * i + 3])
        omega = omega + m
        b = matrix([[-1 * dy / motion_noise], [dy / motion_noise]])
        b = b.expand(nd, 1, [2 * i + 1, 2 * i + 3], [0])
        xi = xi + b
        Z = data[i][0]
        for j in range(len(Z)):
            ...
            m = m.expand(nd, nd, [2 * i, 2 * N + 2 * index], [2 * i, 2 * N + 2 * index])
            omega = omega + m
            b = matrix([[-1 * dx / measurement_noise], [dx / measurement_noise]])
            b = b.expand(nd, 1, [2 * i, 2 * N + 2 * index], [0])
            xi = xi + b
            dy = Z[j][2]
            m = matrix(
                [
            ...
            omega = omega + m
            b = matrix([[-1 * dy / measurement_noise], [dy / measurement_noise]])
            b = b.expand(nd, 1, [2 * i + 1, 2 * N + 2 * index + 1], [0])
            xi = xi + b
    mu = omega.inverse() * xi
