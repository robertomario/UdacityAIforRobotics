...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if(alpha<-1*max_steering_angle):
            alpha=-1*max_steering_angle
        d=motion[1]
        ...
        if(abs(beta)<0.001):
            ...
            r=d/beta
            new_x=self.x+r*(sin(self.orientation+beta)-sin(self.orientation))
            new_y=self.y+r*(cos(self.orientation)-cos(self.orientation+beta))
